...
@foreach ($sub_categories as $sub_category)
    <div data-te-modal-init
        class="fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none"
        id="editSubCategoryModal{{ $sub_category->id }}" tabindex="-1" aria-labelledby="editSubCategoryLabel" aria-hidden="true"
        aria-modal="true">
        <div data-te-modal-dialog-ref
            class="pointer-events-none relative w-auto translate-y-[-50px] opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
            <div
                ...
                <div class="relative flex-auto p-4" data-te-modal-body-ref>
                    <form action="/subkategori/edit/{{ $sub_category->id }}" method="POST">
                        @csrf
                        <div class="grid grid-cols-1 gap-2">
                            <input id="d1" type="hidden" name="kategori_id" value="{{ $sub_category->category_id }}">
                            <label for="">Nama SubKategori</label>
                            <input type="text" class="px-2 py-1 bg-gray-100 border border-1 rounded" name="nama_sub_kategori"
